...
async function getCharactersfor(name, filter = "anime") {
    ...
    try {
        ...
        if (x.hasUrl == true) {
            ...
            //console.log(extraURL)
            try{
                let data = await rp("https://www.anime-planet.com/" + filter + "/" + change(name) + "/characters")
                const $ = cheerio.load(data);
                x = []
                a = $("tr").each(function () {
    ...
    try {
        ...
        if (x.hasUrl == true) {
            ...
            try{
                ...
                result = { resultPresent: false, error : error ,err: 'please retry some error happend. Try changing the name of anime or spelling it correctly' }
                return result;
            }
        }
        else {
